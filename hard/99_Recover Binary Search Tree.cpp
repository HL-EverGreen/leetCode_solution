...
public:
    ...
    void recoverTree(TreeNode* root) {
        // BST
        // 56ms, beats 99%
        // Main idea: inorder BST can get sorted array, so just check each element and its predecessor's value, first time store prev node, second time store root node
